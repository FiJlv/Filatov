...
namespace Homework_1_Filatov_Vladyslav_Serhiyovych
{
    class Storage
    {
        ...
        public event OrderHandler? SatisfiedOrder;
        public event OrderHandler? UnsatisfiedOrder;
        ...
        public void OrderFulfillmentAnalysis(FileHandler fileHandler)
        {
            Order[] orders = fileHandler.ReadOrders();

            var satisfiedOrders = from order in orders
                                    join product in Products on order.ProductName equals product.Name
                                    select new { Name = order.ProductName };

            foreach (var order in satisfiedOrders)
                SatisfiedOrder($"Замовлення на товар '{order.Name}' виконано");

            List<string> ordersList = new List<string>();
            foreach (var item in orders)
                ordersList.Add(item.ProductName);

            List<string> productList = new List<string>();
            foreach (var item in Products)
                productList.Add(item.Name);

            var unsatisfiedOrders = ordersList.Except(productList);

            foreach (string unsatisfiedOrder in unsatisfiedOrders)
            {
                UnsatisfiedOrder($"Товару '{unsatisfiedOrder}' немає на складi, сумiжнi товари записанi в файл");
                fileHandler.SimilarProducts(unsatisfiedOrder);
            }
        }
        ...
        {
            Array.Sort(products, new StorageComparer(sortBy));
